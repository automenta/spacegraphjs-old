	...
	if(rayCallback.hasHit()){
	  var body = rayCallback.get_m_collisionObject();
		  if(body){
		    ...
		    body = Ammo.btRigidBody.prototype.upcast(body);
			var v = body.getCenterOfMassPosition();
			this.m_cameraTargetPosition.op_mul(0);
			this.m_cameraTargetPosition.op_add(v);

			var aa = new btVector3();
			var bb = new btVector3();
			body.getAabb(aa, bb);
			aa.op_sub(bb);
			var d = Math.max(aa.x(), Math.max(aa.y(), aa.z()));
			this.m_cameraDistance = d*10;

			/*
			this.m_cameraUp.op_mul(0);
			this.m_cameraUp.op_add(body.getOrientation().getAxis());
			this.m_cameraUp.op_mul(-1);*/
